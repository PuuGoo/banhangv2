<div class="container">
    <div class="site-home sh">
        <div class="sh-featured_product shfp mt-80">
            <?php $cat = Catelogy::get_catelogy_by_id($id_loai); ?>
            <div class="display-2 text-center mb-40"><?= $cat->ten_loai ?></div>
            <?php $catelogyProduct = $cp = Product::find_product_by_idCatelogy($id_loai, $pageNum, $pageSize); ?>
        ...
        <div class="sh-pagination text-center">
            <?php if ($pageNum >= 2) : ?>
                <a class="display-5 me-16" style="color: var(--darkGreen-01)" href='<?= ROOT_URL . "catelogy?id_loai=$id_loai&page=1" ?>'>First Page</a>
            <?php endif; ?>
            <?php if ($prePage >= 1) : ?>
                <a class="display-5 me-16" style="color: var(--darkGreen-01)" href='<?= ROOT_URL . "catelogy?id_loai=$id_loai&page=$prePage" ?>'>Previous Page</a>
            <?php endif; ?>
            <?php if ($nextPage < $pageTotal) : ?>
                <a class="display-5 me-16" style="color: var(--darkGreen-01)" href='<?= ROOT_URL . "catelogy?id_loai=$id_loai&page=$nextPage" ?>'>Next Page</a>
            <?php endif; ?>
            <?php if ($pageNum < $pageTotal) : ?>
                <a class="display-5 " style="color: var(--darkGreen-01)" href='<?= ROOT_URL . "catelogy?id_loai=$id_loai&page=$pageTotal" ?>'>Last Page</a>
            <?php endif; ?>
        </div>
    </div>
</div>